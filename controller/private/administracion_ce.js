...
// Constantes para establecer las rutas y parámetros de comunicación con la API.
const API_CATEGORIA = SERVER + 'private/categorias.php?action=';
// Método manejador de eventos que se ejecuta cuando el documento ha cargado.
document.addEventListener('DOMContentLoaded', function () {
    // Se llama a la función que obtiene los registros para llenar la tabla. Se encuentra en el archivo components.js
    readRows(API_ESPECIES);
    readSecondRows(API_CATEGORIA);
});

// Función para llenar la tabla con los datos de los registros. Se manda a llamar en la función readRows().
function fillTable(dataset) {
    let content_especie = '';
    let content_categoria = '';
    ...
    dataset.map(function (row) {
        ...
        content_categoria += `
        <tr>
            <th class="text-center align-middle">${row.id_categoria}</th>
            <td class="align-middle">${row.categoria}
                <span class="badge bg-danger" style="display:${row.activo == true ? 'none':''}">Inactivo</span>
            </td>
    ...
    document.getElementById('tbody-rows_especie').innerHTML = content_especie;
    document.getElementById('tbody-rows').innerHTML = content_categoria;
}
